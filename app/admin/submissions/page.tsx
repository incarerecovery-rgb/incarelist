...
export const metadata = {
  title: "Admin — New Profile Submissions",
};
          ...
          {submissions.map((s: any) => (
            <li key={s.id} className="rounded-2xl border border-line bg-white p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <p className="font-semibold text-ink">{s.provider_name}</p>
                  <p className="text-sm text-ink/60">
                    {s.address}, {s.state_slug} {s.zip} · {s.phone}
                  </p>
                  <p className="text-xs text-ink/50 mt-1">Category: {s.category_slug}</p>
                  {s.website && (
                    <a href={s.website} target="_blank" rel="noopener noreferrer" className="text-sm text-navy-600 hover:underline">
                      {s.website}
                    </a>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {(s.insurance_types ?? []).map((t: string) => (
                  <span key={t} className="rounded-full bg-navy-50 px-2.5 py-1 text-xs font-medium text-navy-700">
                    {t}
                  </span>
                ))}
              </div>
