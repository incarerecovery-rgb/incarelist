...
This assumes you've unzipped `incarelist.zip` somewhere on your computer
...
4. Go to Authentication → URL Configuration and add:
   ...
   - your future production URL, e.g. `https://incarelist.com/auth/callback`
...
(you'll see the 17 seeded providers — same ones as before, just from a real
database now). Try the full loop: sign in with magic link → claim a profile
...
```bash
git remote add origin https://github.com/YOUR-USERNAME/incarelist.git
git branch -M main
...
2. Authorize GitHub, select the `incarelist` repo
...
for `incarelist.com` (usually pointing your registrar's DNS to Netlify's
nameservers, or adding the records Netlify shows you).

Once the domain is live, go back to Supabase → Authentication → URL
Configuration and make sure `https://incarelist.com/auth/callback` is in the
allowed redirect list — magic-link sign-in won't work on production without
this.

## 7. Before telling anyone it's live

- Test the full flow on the real production URL: search → provider profile →
  claim → sign-in → admin approve → edit profile
